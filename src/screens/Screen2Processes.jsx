...
export default function Screen2Processes() {
  const { selectedProcesses, setSelectedProcesses, processAnswers, updateProcessAnswer, setStep } = useApp()
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-8 py-10">
      <div className="max-w-3xl mx-auto">
        <ProgressBar step={2} />
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Process Survey</h1>
          <p className="text-gray-500 mt-2">Select the FinOps processes your team handles and rate each on five dimensions.</p>
        </div>

        {/* Upload section */}
        <div className="bg-white border border-dashed border-gray-300 rounded-xl p-5 mb-6 flex items-center justify-between gap-4">
          <div>
          ...
          <div className="flex items-center gap-3">
            {uploadState === 'loading' && <span className="text-xs text-indigo-600 animate-pulse">Analyzing…</span>}
            ...
            <button
              ...
              className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:border-indigo-400 hover:text-indigo-600 transition-colors disabled:opacity-50"
            >
              Choose file
            </button>
